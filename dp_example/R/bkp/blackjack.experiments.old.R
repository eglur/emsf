...
bj.ml <- function(num.episodes = 5e3, epsilon = 0.15, tc = 10, df = 0.999, max.iter = 200, ne = 1e6)
{
...
bj.emsf.full <- function(m, alpha = 1e-2, tcc = NULL, 
                    num.episodes = 5e3, epsilon = 0.15, tc = 10, df = 0.999, max.iter = 200, ne = 1e6) 
{
...
bj.ml.experiment <- function(num.episodes, epsilon, tc, num.avg, load.previous = TRUE, dir = "./files/", ne = 1e6)
{
...
bj.emsf.full.experiment <- function(m, alpha, tcc, num.episodes, epsilon, tc, num.avg, load.previous = TRUE, dir = "./files/", ne = 1e6)
{
  
  if (is.null(tcc)) tcc <- tc
    
  prefix <- paste("bj_emsf_full", num.episodes, epsilon, tc, m, alpha, tcc, sep= "_")
  file.return <- ps(dir, prefix, "_ret.txt")
  file.time   <- ps(dir, prefix, "_tim.txt")
  ...
  if (run <= num.avg)
  {
    for (i in run:num.avg) 
    {
      Y <- bj.emsf.full(m, alpha, tcc, num.episodes, epsilon, tc, ne = ne)
      R[,i] <- Y$R
      T[,i] <- Y$T
...
plot.results <- function(num.episodes, epsilon, tc, emsf.ms, emsf.alpha, emsf.beta, emsf.tcc, qlearning.alpha, dir = "./files/")
{
  
  if (is.null(emsf.tcc)) emsf.tcc <- tc
    
  prefix <- paste("bj_ml", num.episodes, epsilon, tc, sep= "_")
  file.return <- ps(dir, prefix, "_ret.txt")
  file.time   <- ps(dir, prefix, "_tim.txt")

  R <- apply(read.table(file.return), 1, mean)
  T <- apply(read.table(file.time), 1, mean)
...
#   for (i in 1:length(emsf.ms))
#   {
#     prefix <- paste("bj_emsf_full", num.episodes, epsilon, tc, emsf.ms[i], emsf.alpha, emsf.tcc, sep= "_")
#     file.return <- ps(dir, prefix, "_ret.txt")
#     file.time   <- ps(dir, prefix, "_tim.txt")
# 
#     R <- cbind(R, apply(read.table(file.return), 1, mean))
