dyn.load(paste("./c/pole_double", .Platform$dynlib.ext, sep = "")) #melhorar

pole.double.transition <- function(s, a, noise.sd = 0) {
   r <- 0
   g <- 0
   a <- rnorm(1, a, noise.sd)
   R <- .C("pole_double_transition", as.double(t(s)), as.double(a),
